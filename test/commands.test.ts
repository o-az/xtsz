...
it('should run a TypeScript file', async () => {
  ...
  const { stdout } = await execa(xtszExecutable, ['./test/cases/index.ts'])
...
{ userId: 1, id: 1, title: 'delectus aut autem', completed: false }`
  const { stdout } = await execa(xtszExecutable, ['./test/cases/index.cjs'])
  assert.equal(stdout, result)
})

it('should run a file with --file flag', async () => {
  const result = `{ result: { foo: 'bar' } }`
  const { stdout } = await execa(xtszExecutable, ['--file', './test/cases/index.ts'])
  assert.equal(stdout, result)
})

it('should run a file with --url flag', async () => {
  const result = 'bar'
  const { stdout } = await execa(xtszExecutable, [
...
it('should return a help message', async () => {
  const { stdout } = await execa(xtszExecutable, ['--help'])
